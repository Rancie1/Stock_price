...
def train_and_evaluate_model(model, model_name, epochs=20, batch_size=32, X_train=None, y_train=None, X_test=None, y_test=None, metadata=None):
...
if __name__ == "__main__":
    ...
    try:
        ...
        if model1 is None:
            print("❌ Error: Model 1 creation failed - returned None")
        ...
        results1 = train_and_evaluate_model(
            model1, "Simple LSTM", epochs=20, batch_size=16,
            X_train=X_train, y_train=y_train, X_test=X_test, y_test=y_test, metadata=metadata
        )
        ...
        model1 = None  # Set to None if creation failed
    ...
    # Define last_sequence before using it
    last_sequence = metadata['last_sequence']
    ...
    try:
        ...
        # Make prediction - use model1 if available, otherwise skip
        if model1 is not None:
            ...
            print(f"Multivariate prediction result: ${manual_prediction:.2f}")
        else:
            print("❌ No valid model available for multivariate prediction")
    ...
    # Define best_result - assuming we need to use the best model from previous results
    # You may need to adjust this based on your actual model comparison logic
    if model1 is not None:
        best_result = {'model': model1}  # Using model1 as default, adjust as needed
    else:
        print("❌ Error: No valid model available for combined prediction")
        print("Skipping combined prediction analysis...")
        exit()
